...
describe('startGame thunk', () => {
  ...
  it('returns a fulfilled action with the server response', async () => {
    ...
    expect((result as ReturnType<typeof startGame.fulfilled>).payload).toEqual(
      mockGameStartResponse
    );
  });
  ...
  it('updates gameInstance.gameId in the store', async () => {
    ...
    expect(store.getState().gameInstance.gameId).toBe(mockGameStartResponse.gameId);
  });
...
describe('fetchQuests thunk', () => {
  ...
  it('GETs /:gameId/messages', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(`${TEST_API_URL}/${gameId}/messages`);
  });
  ...
  it('returns a fulfilled action with the quest array', async () => {
    ...
    expect((result as ReturnType<typeof fetchQuests.fulfilled>).payload).toEqual(mockQuests);
  });
...
describe('acceptQuest thunk', () => {
  ...
  it('returns a fulfilled action with the quest response', async () => {
    ...
    expect((result as ReturnType<typeof acceptQuest.fulfilled>).payload).toEqual(
      mockQuestResponse
    );
  });
...
describe('purchaseItem thunk', () => {
  ...
  it('returns a fulfilled action with the purchase response', async () => {
    ...
    expect((result as ReturnType<typeof purchaseItem.fulfilled>).payload).toEqual(
      mockPurchaseResponse
    );
  });
...
describe('investigateReputation thunk', () => {
  ...
  it('returns a fulfilled action with the reputation data', async () => {
    ...
    expect((result as ReturnType<typeof investigateReputation.fulfilled>).payload).toEqual(
      mockReputation
    );
  });
  ...
  it('updates reputation in the store', async () => {
    ...
    expect(store.getState().gameInstance.reputation).toEqual(mockReputation);
  });
  ...
  it('increments the turn counter', async () => {
    ...
    expect(store.getState().gameInstance.gameState.turn).toBe(turnBefore + 1);
  });
});
