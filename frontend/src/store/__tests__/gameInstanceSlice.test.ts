import { gameInstanceSlice, gameOver, selectGameId } from '../gameInstanceSlice';
import {
  startGame,
  acceptQuest,
  fetchQuests,
...
describe('gameInstanceSlice', () => {
  ...
  describe('initial state', () => {
    it('matches the expected empty game instance', () => {
      expect(reducer(undefined, { type: '@@INIT' })).toEqual(expectedInitial);
    });
  });
  ...
  describe('fetchQuests.fulfilled', () => {
    it('stores the returned quests array', () => {
      const action = fetchQuests.fulfilled(mockQuests, 'reqId', 'game-abc');
      expect(reducer(undefined, action).quests).toEqual(mockQuests);
    });

    it('replaces any existing quests', () => {
      const first = fetchQuests.fulfilled([mockQuests[0]], 'req1', 'game-abc');
      const second = fetchQuests.fulfilled([mockQuests[1]], 'req2', 'game-abc');
  ...
  describe('acceptQuest.fulfilled', () => {
    it('merges the quest response into gameState', () => {
      const action = acceptQuest.fulfilled(
        mockQuestResponse,
        'reqId',
        { gameId: 'game-abc', adId: 'q1' }
      );
      const state = reducer(undefined, action);
    ...
    it('does NOT set gameOver when lives > 0', () => {
      ...
      const action = acceptQuest.fulfilled(response, 'reqId', { gameId: 'g', adId: 'q' });
  ...
  describe('purchaseItem.fulfilled', () => {
    ...
    it('does NOT set gameOver when lives > 0', () => {
      ...
      const action = purchaseItem.fulfilled(response, 'reqId', { gameId: 'g', itemId: 'i' });
      expect(reducer(undefined, action).gameOver).toBe(false);
    });
  ...
  describe('investigateReputation.fulfilled', () => {
    it('stores the returned reputation', () => {
      const action = investigateReputation.fulfilled(mockReputation, 'reqId', 'game-abc');
      expect(reducer(undefined, action).reputation).toEqual(mockReputation);
    });
