...
describe('apiUrlSlice', () => {
  ...
  describe('initial state', () => {
    it('derives the API URL from the injected import.meta.env stubs', () => {
      ...
      expect(reducer(undefined, { type: '@@INIT' }).apiURL).toBe(expected);
    });
  });
  ...
  describe('setApiUrl', () => {
    ...
    it('overwrites a previously set URL', () => {
      const first = reducer(undefined, setApiUrl('http://first.example.com'));
      const second = reducer(first, setApiUrl('http://second.example.com'));
